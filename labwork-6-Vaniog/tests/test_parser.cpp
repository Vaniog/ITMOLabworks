...
TEST(ParserTestSuite, FileTest){
    std::filesystem::path path(L"example/config.omfl");
    const auto root = parse(path);

    ASSERT_TRUE(root.valid());

    ASSERT_EQ(root.Get("common").Get("name").AsString(), "Common config");
    ASSERT_EQ(root.Get("common").Get("description").AsString(), "Some config");
    ASSERT_EQ(root.Get("common.version").AsInt(), 1);
    ASSERT_EQ(root.Get("servers.first").Get("ports")[0].AsInt(), 100505);
    ASSERT_EQ(root.Get("servers.first").Get("ports")[1].AsInt(), 10506);
}